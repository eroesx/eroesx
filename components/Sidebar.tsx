...
const rolePermissions: Record<UserRole, Page[]> = {
  'Yönetici': ['dashboard', 'admin', 'blockManagement', 'plateInquiry', 'cashManagement', 'duesManagement', 'neighbors', 'announcements', 'feedback', 'users', 'expenses', 'settings'],
  'Daire Sahibi': ['dashboard', 'plateInquiry', 'announcements', 'neighbors', 'feedback', 'dues'],
  'Kiracı': ['dashboard', 'plateInquiry', 'announcements', 'neighbors', 'feedback', 'dues'],
};
...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, isSidebarOpen, setSidebarOpen, currentUser, onLogoDoubleClick, isResidentViewMode = false, feedbacks, messages, onLogout }) => {
  ...
  if (currentUser.role === 'Yönetici' && isResidentViewMode) {
      const hiddenPages: Page[] = ['admin', 'blockManagement', 'users', 'expenses', 'settings', 'duesManagement', 'cashManagement'];
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto">
          {navItems.map((item) => {
             let badgeCount = 0;
             if (item.page === 'feedback') badgeCount = unreadFeedbackCount;
