...
const BankIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 14v20c0 4.418 7.163 8 16 8 1.381 0 2.721-.087 4-.252M8 14c0 4.418 7.163 8 16 8s16-3.582 16-8M8 14c0-4.418 7.163-8 16-8s16 3.582 16 8m0 0v14m0-4c0 4.418-7.163 8-16 8S8 28.418 8 24m32 10c0 4.418-7.163 8-16 8S8 38.418 8 34" /></svg>;
...
const ChatIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z" /></svg>;
const ClipboardListIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>;
const IdentificationIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 012-2h2a2 2 0 012 2v1m-4 0h4m-4 0a2 2 0 100 4h4a2 2 0 100-4h-4z" /></svg>;
...
const NavLink: React.FC<{
  ...
  >
    ...
    {badge !== undefined && badge > 0 && (
        <span className="px-2 py-0.5 text-[10px] font-black text-white bg-indigo-500 rounded-full shadow-lg animate-pulse ring-2 ring-indigo-400/20">
            {badge}
        </span>
    )}
  </a>
...
const navItemDefinitions: { page: Page; label: string; icon: React.ReactNode }[] = [
    ...
    { page: 'cashManagement', label: 'Kasa & Banka', icon: <WalletIcon /> },
    { page: 'duesManagement', label: 'Aidat Takibi', icon: <CashIcon /> }, 
    { page: 'expenses', label: 'Giderler', icon: <ChartBarIcon /> },
    { page: 'reports', label: 'Raporlar', icon: <ChartPieIcon /> },
    { page: 'dues', label: 'Aidat Geçmişim', icon: <CashIcon /> }, 
    { page: 'neighbors', label: 'Komşular', icon: <ChatIcon /> },
    { page: 'announcements', label: 'Bildirimler', icon: <ClipboardListIcon /> }, // 'Duyurular' kaldırıldı, Form 'Bildirimler' olarak adlandırıldı ve bu sayfaya bağlandı.
    { page: 'users', label: 'Kullanıcılar', icon: <UsersIcon /> },
    { page: 'allUsers', label: 'Tüm Sakinler Listesi', icon: <IdentificationIcon /> },
    { page: 'settings', label: 'Ayarlar', icon: <CogIcon /> },
];

const rolePermissions: Record<UserRole, Page[]> = {
  'Yönetici': ['dashboard', 'admin', 'blockManagement', 'plateInquiry', 'cashManagement', 'duesManagement', 'neighbors', 'announcements', 'users', 'allUsers', 'expenses', 'reports', 'settings'],
  'Daire Sahibi': ['dashboard', 'plateInquiry', 'neighbors', 'announcements', 'dues'],
  'Kiracı': ['dashboard', 'plateInquiry', 'neighbors', 'announcements', 'dues'],
  'Denetçi': ['dashboard', 'plateInquiry', 'neighbors', 'announcements', 'dues', 'expenses', 'reports'],
};
...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, isSidebarOpen, setSidebarOpen, currentUser, onLogoDoubleClick, isResidentViewMode = false, feedbacks, messages, onLogout, connections, appVersion }) => {
  ...
  if (currentUser.role === 'Yönetici' && isResidentViewMode) {
      const hiddenPages: Page[] = ['admin', 'blockManagement', 'users', 'expenses', 'settings', 'duesManagement', 'cashManagement', 'reports', 'allUsers'];
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="flex items-center justify-center w-12 h-12 bg-indigo-500 rounded-full text-white shadow-lg shadow-indigo-500/30">
             <BuildingIcon />
          </div>
          <div className="text-center mt-3">
              <h2 className="text-lg font-bold text-white tracking-tight leading-none">Site Yönetimi</h2>
              <span className="text-[9px] font-mono text-indigo-300/60 tracking-widest">{appVersion}</span>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto">
          {navItems.map((item) => {
             let badgeCount = 0;
             if (item.page === 'announcements') badgeCount = unreadFeedbackCount;
             if (item.page === 'neighbors') badgeCount = unreadMessageCount;
        ...
        <div className="pt-4 border-t border-gray-700 space-y-1">
            <button
                ...
                className={`flex items-center w-full px-4 py-3 transition-colors duration-200 transform rounded-lg hover:bg-gray-700 group ${currentPage === 'profile' ? 'bg-gray-700 text-white font-semibold' : 'text-gray-400'}`}
            >
                <div className="flex items-center">
                    <UserIcon />
                    <span className="mx-4 font-medium">Profil Bilgilerim</span>
                </div>
            </button>
            <button
                onClick={onLogout}
                className="flex items-center w-full px-4 py-3 text-gray-400 transition-colors duration-200 transform rounded-lg hover:bg-gray-700"
            >
                <LogoutIcon />
