...
const Neighbors: React.FC<NeighborsProps> = (props) => {
    const { currentUser, users, blocks, connections, messages, onSendRequest, onUpdateStatus, onSendMessage, onMarkAsRead } = props;
    ...
    }, [messages, selectedFriendId, activeTab, onMarkAsRead]);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="ml-4 overflow-hidden flex-1">
                                            ...
                                            {lastMsg && <p className={`text-xs truncate mt-1 ${unreadCount > 0 ? 'text-gray-900 font-bold' : 'text-gray-500'}`}>{lastMsg.senderId === currentUser.id ? 'Siz: ' : ''}{lastMsg.content}</p>}
                                        </div>
                                    </div>
                                );
                            })}
                            ...
                            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-1 duration-200`}>
                                <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl shadow-sm text-sm ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white text-gray-700 rounded-tl-none border border-gray-100'}`}>
                                    ...
                                    <div className="flex justify-end items-center mt-1 space-x-1">
                                        ...
                                        {isMe && (
                                            <svg className={`w-3 h-3 ${msg.read ? 'text-green-400' : 'text-indigo-200'}`} fill="currentColor" viewBox="0 0 20 20">
