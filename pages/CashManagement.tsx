...
const CashManagement: React.FC<CashManagementProps> = ({ siteInfo, expenses, dues, users, onUpdateSiteInfo, setCurrentPage }) => {
    ...
    // FIX: Ensure expense amount is treated as a number, defaulting to 0 if it's not a valid number, to prevent type errors.
    ...
    const currentMonthPending = useMemo(() => {
        ...
        // Get eligible users (Active, Not Admin, Not Exempt)
        const eligibleUsers = users.filter(u => u.isActive && u.role !== 'Yönetici' && !u.isDuesExempt);

        let pendingSum = 0;

        eligibleUsers.forEach(user => {
            const userDue = dues.find(d => d.userId === user.id && d.month === currentMonthStr);
            if (userDue) {
                // If record exists and is unpaid, add amount. If paid, add 0.
                if (userDue.status !== 'Ödendi') {
                    pendingSum += userDue.amount;
                }
            } else {
                // If no record exists, they owe the default amount
                pendingSum += siteInfo.duesAmount;
            }
        });

        return pendingSum;
    }, [dues, users, siteInfo.duesAmount]);

    // Calculate Past Years Receivables (Previous years debt)
    const pastYearsReceivables = useMemo(() => {
        const currentYear = new Date().getFullYear();
        const lookbackYears = siteInfo.pastDebtLookbackYears || 2; // Default to 2 if not set
        ...
        eligibleUsers.forEach(user => {
            for (let year = startYear; year < currentYear; year++) {
                months.forEach(month => {
                    ...
                    const record = dues.find(d => d.userId === user.id && d.month === monthStr);
                    
                    if (record) {
                        if (record.status === 'Ödenmedi') {
                            totalPastDebt += record.amount;
                        }
                    } else {
                        // If no record exists for a past month, assume debt
                        totalPastDebt += siteInfo.duesAmount;
                    }
                });
            }
        });

        return totalPastDebt;
    }, [dues, users, siteInfo.duesAmount, siteInfo.pastDebtLookbackYears]);
        ...
        <div className="max-w-5xl mx-auto space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-indigo-600 p-6 rounded-3xl text-white shadow-xl relative overflow-hidden flex flex-col justify-between h-48">
                    ...
                    <div>
                        <p className="text-3xl lg:text-4xl font-black">₺{netBalance.toLocaleString()}</p>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-between h-48">
                    <div className="flex justify-between items-start">
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Geçmiş Yıllar Alacak</h3>
                        <div className="p-1.5 bg-rose-50 rounded-lg text-rose-500">
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        </div>
                    </div>
                    <div>
                        <p className="text-3xl font-black text-rose-600">₺{pastYearsReceivables.toLocaleString()}</p>
                        <p className="text-[10px] font-bold mt-2 text-gray-400 uppercase">Son {siteInfo.pastDebtLookbackYears || 2} Yıldan Devreden</p>
                    </div>
                </div>

                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-between h-48">
                    <div className="flex justify-between items-start">
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Bu Ay Bekleyen</h3>
                        <div className="p-1.5 bg-amber-50 rounded-lg text-amber-500">
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                        </div>
                    </div>
                    <div>
                        <p className="text-3xl font-black text-amber-500">₺{currentMonthPending.toLocaleString()}</p>
                        <p className="text-[10px] font-bold mt-2 text-gray-400 uppercase">Bu Ay Tahsil Edilecek</p>
                    </div>
                </div>
            </div>
                    ...
                    <form onSubmit={handleSave} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea 
                                ...
                                placeholder="Örn: [Blok] Daire [No] [İsim] [Ay] [Yıl] Aidat Ödemesi"
                            />
                            <p className="text-[9px] text-gray-400 font-medium pl-1">
                                * Sakin panelinde otomatik doldurulması için: [Blok], [Daire], [No], [İsim], [Ay], [Yıl] etiketlerini kullanabilirsiniz.
                            </p>
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold mt-2 text-gray-400 uppercase">İşlenen Tüm Giderler</p>
                        </div>
                    </div>
