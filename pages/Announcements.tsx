...
import { Announcement, User, Feedback, FeedbackType, Block, SiteInfo } from '../types';
...
interface AnnouncementsProps {
  ...
  // Feedback related props added for integration
  feedbacks: Feedback[];
  users: User[];
  blocks: Block[];
...
const Announcements: React.FC<AnnouncementsProps> = (props) => {
  const { announcements, currentUser, onUpdate, onDelete, onAdd, isResidentViewMode } = props;
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      {/* Tab Navigation */}
      <div className="flex p-1.5 bg-white rounded-3xl shadow-sm border border-gray-100 mb-8 space-x-2">
          <button 
              onClick={() => setActiveTab('list')}
              className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'list' ? 'bg-indigo-600 text-white shadow-lg scale-[1.02]' : 'text-gray-400 hover:bg-gray-50'}`}
          >
          ...
          <button 
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'feedback' ? 'bg-indigo-600 text-white shadow-lg scale-[1.02]' : 'text-gray-400 hover:bg-gray-50'}`}
          >
        ...
        {activeTab === 'list' ? (
          <>
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-8">
                <h2 className="text-xl font-black text-gray-800 uppercase tracking-tight">Duyuru Listesi</h2>
                {isManager && (
                    <button 
          ...
          <FeedbackPage 
            ...
            isResidentViewMode={props.isResidentViewMode}
            onUpdateDues={props.onUpdateDues}
