...
const AdminPanel: React.FC<AdminPanelProps> = ({ onAddAnnouncement, setCurrentPage, siteInfo, onUpdateSiteInfo, onSeedDatabase }) => {
    ...
    const handleInfoSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onUpdateSiteInfo({ duesAmount, iban, bankName, note });
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <p className="text-gray-600 mt-2">Sistem genelindeki kritik ayarlara buradan erişebilirsiniz.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-indigo-500">
            ...
            <form onSubmit={handleInfoSubmit} className="space-y-3">
                ...
                <input type="text" value={iban} onChange={(e) => setIban(e.target.value)} className="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg" placeholder="IBAN" />
                <button type="submit" className="w-full py-2 bg-indigo-600 text-white rounded-lg font-bold">Bilgileri Güncelle</button>
            </form>
        </div>
      </div>
